...
public:
            ...
            ParamServer("lio_sam_imu_preintegration", options)
    {
        ...
        priorPoseNoise  = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 1e-5, 1e-5, 1e-5, 1e-5, 1e-5, 1e-5).finished()); // rad,rad,rad,m, m, m
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw)
    {
        ...
        gtsam::Values result = optimizer.calculateEstimate();
        gtsam::Matrix covarianceMatrix = optimizer.marginalCovariance(X(key-1));
